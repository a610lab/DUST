...
import ramps
from data.dataset1 import BaseDataSets, RandomGenerator, TwoStreamBatchSampler
from loss import DiceLoss
from model.unet import UNet_URPC
from utils_2d import test_single_volume

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,
                    default=r'/home/a610/Project/Qzh/Prostate5/Prostate', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='Prostate/Uncertainty_Rectified_Pyramid_Consistency', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    model = UNet_URPC(in_chns=1, class_num=num_classes).cuda()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            consistency_dist_show = (torch.mean(consistency_dist_main) + torch.mean(consistency_dist_aux1) +
                                torch.mean(consistency_dist_aux2) + torch.mean(consistency_dist_aux3)) / 4
            ...
            iter_num = iter_num + 1
            logging.info(
                'iteration %d : loss : %03f, supervised_loss: %03f, consistency_loss: %03f， consistency_dist: %03f' %
                (iter_num, loss, supervised_loss, consistency_loss, consistency_dist_show))
