...
from model.unet1 import UNet_UMMC_v2
...
def main(args, model_path1, model_path2, model_path3):
    ...
    model = UNet_UMMC_v2(in_chns=1, class_num=args.num_classes).cuda()
    ...
    semi_model = UNet_UMMC_v2(in_chns=1, class_num=args.num_classes).cuda()
    ...
    logger.info('Pseudo label dice : {}'.format(Dice))
    final_model = UNet_UMMC_v2(in_chns=1, class_num=args.num_classes).cuda()
...
def pretrain(model, optimizer, trainloader, valloader, db_val, args, model_path1):
    ...
    for epoch in tqdm(range(1, pretraining_epochs + 1), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss = (loss_ce + loss_ce_aux1 + loss_dice + loss_ce_aux2) / 4
            ...
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr_
            logger.info('epoch %d : loss : %03f, loss_ce: %03f, loss_ce_aux1: %03f, loss_dice: %03f, loss_ce_aux2: %03f'
                        % (epoch, loss, loss_ce, loss_ce_aux1, loss_dice, loss_ce_aux2))
    ...
    for epoch in tqdm(range(0, semitraining_epochs), ncols=70):
        if epoch % save_step == 0 and epoch > 0:
            ...
            if performance > best_performance:
                ...
                save_mode_path = os.path.join(model_path2, 'epoch_{}_dice_{}.pth'.format(epoch, round(best_performance, 4)))
                torch.save(model.state_dict(), save_mode_path)
                torch.save(model.state_dict(), save_best)
                best_model = deepcopy(model)
            logger.info('epoch %d : mean_dice : %f mean_hd95 : %f' % (epoch, performance, mean_hd95))
        ...
        if epoch % pred_step == 0 and epoch >= 10:
            logger.info('Starting predict unlab')
            new_loader, Dice1 = pred_unlabel(model, semitrainloader, args.batch_size)
        ...
        for i_batch, ((labeled_batch), (unlabeled_batch)) in enumerate(zip(trainloader, new_loader)):
            ...
            supervised_loss = (loss_ce + loss_ce_aux1 + loss_dice + loss_ce_aux2) / 4

            '''UnSupervised Loss'''
            un_outputs, un_outputs_aux1, un_outputs_aux2 = model(unvolume_batch)
            ...
            variance = torch.sum(kl_distance(
                torch.log(un_outputs_soft + 1e-8), un_outputs_aux2_soft), dim=1, keepdim=True)
            exp_variance = torch.exp(-variance)

            pseudo_supervision_loss = torch.mean(
                CE_loss_r(un_outputs_aux1, pred_batch[:].long()) * exp_variance) + torch.mean(variance)

            '''balance loss'''
            balance_loss = abs(ce_loss(un_outputs_aux2, pred_batch[:].long()) - ce_loss(outputs_aux2, label_batch[:].long()))

            loss = supervised_loss + pseudo_supervision_loss + balance_loss
            ...
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr_
            logger.info('epoch %d : loss : %03f, supervised_loss: %03f, pseudo_supervision_loss: %03f, balance_loss: %03f'
                % (epoch, loss, supervised_loss, pseudo_supervision_loss, balance_loss))
    ...
    for epoch in tqdm(range(0, finaltraining_epochs), ncols=70):
        if epoch % save_step == 0 and epoch > 0:
            ...
            if performance > best_performance:
                ...
                save_mode_path = os.path.join(model_path3, 'epoch_{}_dice_{}.pth'.format(epoch, round(best_performance, 4)))
                torch.save(model.state_dict(), save_mode_path)
                torch.save(model.state_dict(), save_best)
                best_model = deepcopy(model)
            logger.info('epoch %d : mean_dice : %f mean_hd95 : %f' % (epoch, performance, mean_hd95))
        ...
        if epoch % pred_step == 0 and epoch >= 10:
            logger.info('Starting predict unlab')
            new_loader, Dice1 = pred_unlabel(model, finaltrainloader, args.batch_size)
            logger.info('Pseudo label dice : {}'.format(Dice1))
